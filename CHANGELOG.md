...
- `scripts/check-trailer` (+ `check_trailer.py`) — cross-repo same-task enforcement: a commit-msg hook installed in *product* repos; staged paths matched against the knowledge repo's `affects:` globs (incl. `external:` repo-name matching) require a `Knowledge:` trailer or `no knowledge impact:` line. Closes the seam drift-check cannot see.
- **What-cache / why-asset** (seeded as a methodology principle): budget article prose by the derivability test — what-sections are caches of the code (thin facts + code-cite pointers; recurring invalidation bill), why-sections are primary sources (rejected alternatives, constraints, negative results, field incidents — zero maintenance, appreciating). Design: `knowledge/concepts/methodology/what-cache-why-asset.md`; glossary entry + overview decision-rule-4 pointer.
