...
`index.md` and `log.md` are skipped by name, and everything under
`log/` (rolled archives) and `facts/` (generated by build-facts, carries
a sentinel not frontmatter) is skipped by directory. They're index/log
